...
const Poster = () => {
  ...
  function getPosterZoomConfig(posterId) {
    const configs = {
      ...
      6: {
        ...
        zoomSize: "w-35 2xl:w-48",
        zoomHeight: "h-2 2xl:h-4",
  ...
  return (
    <div className="min-h-screen w-full relative overflow-hidden" style={{ backgroundColor: '#1D1C1A' }}>
      ...
      <button
        className="fixed right-6 top-1/2 transform -translate-y-1/2 2xl:right-8 transition-opacity z-50 w-[33px] h-[49px] 2xl:w-[44px] 2xl:h-[66px]"
      ...
      >
        {isHoveringArrowButton ? (
          <svg className="w-[41px] h-[61px] 2xl:w-[55px] 2xl:h-[82px]" viewBox="0 0 41 61" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <svg className="w-[33px] h-[49px] 2xl:w-[44px] 2xl:h-[66px]" viewBox="0 0 33 49" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {selectedPoster && (
        ...
        >
          <div 
            className={`relative w-full h-auto bg-transparent ${selectedPoster === 6 ? 'max-w-3xl 2xl:max-w-5xl' : 'max-w-2xl 2xl:max-w-4xl'}`}
          >
                          <img
                src={`/poster/pictures/zoomIn/${selectedPoster.toString().padStart(2, '0')}.png`}
                alt={`Poster ${selectedPoster}`}
                className={`w-full h-auto object-cover ${selectedPoster === 6 ? 'max-h-[20vh] 2xl:max-h-[30vh]' : selectedPoster === 8 ? 'max-h-[10vh] 2xl:max-h-[15vh] max-w-[470px] 2xl:max-w-[630px]' : 'max-h-[70vh] 2xl:max-h-[80vh]'} border border-white shadow-2xl`}
            ...
            {selectedPoster === 1 && (
              ...
              >
                <div className="text-left">
                  <div className="font-bold text-xl 2xl:text-3xl mb-1 2xl:mb-2">THE SILENCE OF THE LAMBS, 1991</div>
                  <div className="font-normal text-base 2xl:text-xl opacity-70 text-gray-300">The skull on the moth's back is made of seven nude</div>
                  <div className="font-normal text-base 2xl:text-xl opacity-70 text-gray-300">women, in a composition based on Salvador Dalí's art.</div>
                </div>
              </div>
            )}
            {selectedPoster === 2 && (
              ...
              >
                <div className="text-left">
                  <div className="font-bold text-xl 2xl:text-3xl mb-1 2xl:mb-2">ONCE UPON A TIME IN HOLLYWOOD, 2019</div>
                  <div className="font-normal text-base 2xl:text-xl opacity-70 text-gray-300">Quentin Tarantino, the director, appears with a camera,</div>
                  <div className="font-normal text-base 2xl:text-xl opacity-70 text-gray-300">even though he doesn't appear in the film itself.</div>
                </div>
              </div>
            )}
            {selectedPoster === 3 && (
              ...
              >
                <div className="text-left">
                  <div className="font-bold text-xl 2xl:text-3xl mb-1 2xl:mb-2">UNDER THE SILVER LAKE, 2018</div>
                  <div className="font-normal text-base 2xl:text-xl opacity-70 text-gray-300">Hidden figures like a pirate and a mermaid appear throughout</div>
                  <div className="font-normal text-base 2xl:text-xl opacity-70 text-gray-300">the poster, a nod to the film's theme of hidden clues.</div>
                </div>
              </div>
            )}
            {selectedPoster === 4 && (
              ...
              >
                <div className="text-left">
                  <div className="font-bold text-xl 2xl:text-3xl mb-1 2xl:mb-2">PASSENGERS, 2016</div>
                  <div className="font-normal text-base 2xl:text-xl opacity-70 text-gray-300">The lines and circles are arranged in Morse code that spells out "SOS",</div>
                  <div className="font-normal text-base 2xl:text-xl opacity-70 text-gray-300">a hint at the film's characters' distress.</div>
                </div>
              </div>
            )}
          </div>
