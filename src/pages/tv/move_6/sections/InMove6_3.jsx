...
const InMove6_3 = () => {
  ...
  useEffect(() => {
    const loadImages = async () => {
      const [mainImg, hoverImg, zoomImg] = await Promise.all([
        getBase64('/tv/pictures/tv2/move-6/regular/03.png'),
        getBase64('/tv/pictures/tv2/move-6/zoomBit/03.png'),
        getBase64('/tv/pictures/tv2/move-6/zoomIn/03.png')
      ]);
      setImage(mainImg);
  ...
  const imageZoomConfigs = useMemo(() => ({
    3: { zoomSize: 'w-[521px]', zoomHeight: 'h-[844px]', zoomOffset: { x: -530, y: 0 } }
  }), []);
  ...
  const handleImageMouseMove = (event) => {
    ...
    const inClickableArea = isPointInTvMove6Area(3, mouseX, mouseY, rect.width, rect.height);
    
    if (inClickableArea) {
      // setHoveredImage(3); // This line is removed
    } else {
      // setHoveredImage(null); // This line is removed
    }
  };
  ...
  const renderClickableAreasDebug = () => {
    ...
    const areas = getTvMove6ClickableAreas(3);
  ...
  return (
    <div className="relative w-full h-screen" style={{ backgroundColor: '#1D1C1A' }}>
      ...
      >
        <img 
          src={image || "/tv/pictures/tv2/move-6/regular/03.png"}
          alt="Full Screen Scene"
          className="w-full h-full object-cover"
        />
        {renderClickableAreasDebug()}
        {/* Add clickable area overlay */}
        {getTvMove6ClickableAreas(3).map((area, index) => (
          <div
            key={`clickable-${index}`}
      ...
      {((isAreaHovered || isHoveringZoomImage)) && (() => {
        ...
        return (
          ...
          >
            <img
              src={hoverImage || "/tv/pictures/tv2/move-6/zoomBit/03.png"}
        ...
        >
          <div className="relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col items-center">
              {/* Image */}
              <img
                src={zoomImage || "/tv/pictures/tv2/move-6/zoomIn/03.png"}
                alt="Zoomed Scene"
                style={{
                  width: '582px',
                  height: '663px',
