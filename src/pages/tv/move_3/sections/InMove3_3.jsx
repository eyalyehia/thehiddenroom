...
const InMove3_3 = () => {
  ...
  useEffect(() => {
    const loadImages = async () => {
      const [mainImg, hoverImg, zoomImg] = await Promise.all([
        ...
        getBase64('/tv/pictures/tv1/move-3/zoomBit/03.png'),
        getBase64('/tv/pictures/tv1/move-3/zoomIn/03.png')
      ]);
      setImage(mainImg);
  ...
  const imageZoomConfigs = useMemo(() => ({
    3: { zoomSize: 'w-[394px]', zoomHeight: 'h-[40px]', zoomOffset: { x: -150, y: -269 } }
  }), []);
  ...
  const handleImageMouseMove = (event) => {
    ...
    const inClickableArea = isPointInTvMove3Area(3, mouseX, mouseY, rect.width, rect.height);
  ...
  const renderClickableAreasDebug = () => {
    ...
    const areas = getTvMove3ClickableAreas(3);
  ...
  return (
    <div className="relative w-full h-screen" style={{ backgroundColor: '#1D1C1A' }}>
      ...
      >
        <img 
          src={image || "/tv/pictures/tv1/move-3/regular/03.png"}
          alt="Full Screen Scene"
          className="w-full h-full object-cover"
        />
        {renderClickableAreasDebug()}
        {/* Add clickable area overlay */}
        {getTvMove3ClickableAreas(3).map((area, index) => (
          <div
            key={`clickable-${index}`}
      ...
      {hoveredImage && (() => {
        ...
        return (
          ...
          >
            <img
              src={hoverImage || "/tv/pictures/tv1/move-3/zoomBit/03.png"}
              alt="Hover Scene"
      ...
      {showModal && (
        ...
        >
          <div className="relative" onClick={e => e.stopPropagation()}>
            ...
            {/* Main Image */}
            <img
              src={zoomImage || "/tv/pictures/tv1/move-3/zoomIn/03.png"}
              alt="Zoomed Scene"
              className="w-[676px] h-[663px] object-cover"
            />
            ...
            <div className="mt-4 text-white" style={{position: 'absolute', left: 0, bottom: '-85px', textAlign: 'left'}}>
              ...
              >
                The main character answers a public phone labeled "No incoming calls", a hint that Tyler may not be real.
              </p>
            </div>
          </div>
        </div>
