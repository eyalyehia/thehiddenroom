...
const InMove3_2 = () => {
  ...
  useEffect(() => {
    const loadImages = async () => {
      const [mainImg, hoverImg, zoomImg] = await Promise.all([
        ...
        getBase64('/tv/pictures/tv1/move-3/zoomBit/02.png'),
        getBase64('/tv/pictures/tv1/move-3/zoomIn/02.png')
      ]);
      setImage(mainImg);
  ...
  const imageZoomConfigs = useMemo(() => ({
    2: { zoomSize: 'w-[301px]', zoomHeight: 'h-[581px]', zoomOffset: { x: -190, y: -95 } }
  }), []);
  ...
  const handleImageMouseMove = (event) => {
    ...
    const inClickableArea = isPointInTvMove3Area(2, mouseX, mouseY, rect.width, rect.height);
    
    if (inClickableArea) {
      // setHoveredImage(2); // This line was removed as per the edit hint.
    } else {
  ...
  const renderClickableAreasDebug = () => {
    ...
    const areas = getTvMove3ClickableAreas(2);
  ...
  return (
    <div className="relative w-full h-screen" style={{ backgroundColor: '#1D1C1A' }}>
      ...
      >
        <img 
          src={image || "/tv/pictures/tv1/move-3/regular/02.png"}
          alt="Full Screen Scene"
          className="w-full h-full object-cover"
        />
        {renderClickableAreasDebug()}
        {/* Add clickable area overlay */}
        {getTvMove3ClickableAreas(2).map((area, index) => (
          <div
            key={`clickable-${index}`}
      ...
      {((isAreaHovered || isHoveringZoomImage)) && (() => {
        ...
        return (
          ...
          >
            <img
              src={hoverImage || "/tv/pictures/tv1/move-3/zoomBit/02.png"}
      ...
      {showModal && (
        ...
        >
          <div className="relative" onClick={e => e.stopPropagation()}>
            ...
            {/* Main Image */}
            <img
              src={zoomImage || "/tv/pictures/tv1/move-3/zoomIn/02.png"}
              alt="Zoomed Scene"
              className="w-[554px] h-[663px] object-cover"
            />
            ...
            <div className="mt-4 text-white" style={{position: 'absolute', left: 0, bottom: '-105px', textAlign: 'left'}}>
              ...
              >
                Before Tyler Durden is officially introduced, Brad Pitt flashes on screen for a split second, blink and you'll miss him.
              </p>
            </div>
          </div>
        </div>
