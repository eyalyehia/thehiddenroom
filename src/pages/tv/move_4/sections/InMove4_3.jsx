...
const InMove4_3 = () => {
  ...
  useEffect(() => {
    const loadImages = async () => {
      const [mainImg, hoverImg, zoomImg] = await Promise.all([
        ...
        getBase64('/tv/pictures/tv1/move-4/zoomBit/03.png'),
        getBase64('/tv/pictures/tv1/move-4/zoomIn/03.png')
      ]);
      setImage(mainImg);
  ...
  const imageZoomConfigs = useMemo(() => ({
    3: { zoomSize: 'w-[241px]', zoomHeight: 'h-[209px]', zoomOffset: { x: -760, y: 400 } }
  }), []);
  ...
  const handleImageMouseMove = (event) => {
    ...
    const inClickableArea = isPointInTvMove4Area(3, mouseX, mouseY, rect.width, rect.height);
  ...
  const renderClickableAreasDebug = () => {
    ...
    const areas = getTvMove4ClickableAreas(3);
  ...
  return (
    <div className="relative w-full h-screen" style={{ backgroundColor: '#1D1C1A' }}>
      ...
      >
        <img 
          src={image || "/tv/pictures/tv1/move-4/regular/03.png"}
          alt="Full Screen Scene"
          className="w-full h-full object-cover"
        />
        {renderClickableAreasDebug()}
        {/* Add clickable area overlay */}
        {getTvMove4ClickableAreas(3).map((area, index) => (
          <div
            key={`clickable-${index}`}
      ...
      {((isAreaHovered || isHoveringZoomImage)) && (() => {
        ...
        return (
          ...
          >
            <img
              src={hoverImage || "/tv/pictures/tv1/move-4/zoomBit/03.png"}
        ...
        >
          <div className="relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col items-center">
              {/* Image */}
              <img
                src={zoomImage || "/tv/pictures/tv1/move-4/zoomIn/03.png"}
                alt="Zoomed Scene"
                style={{
                  width: '905px',
                  height: '663px',
