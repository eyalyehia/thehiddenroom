...
const InMove5_4 = () => {
  ...
  useEffect(() => {
    const loadImages = async () => {
      const [mainImg, hoverImg, zoomImg] = await Promise.all([
        ...
        getBase64('/tv/pictures/tv2/move-5/zoomBIt/04.png'),
        getBase64('/tv/pictures/tv2/move-5/zoomIn/04.png')
      ]);
      setImage(mainImg);
  ...
  const imageZoomConfigs = useMemo(() => ({
    4: { zoomSize: 'w-[295px]', zoomHeight: 'h-[24px]', zoomOffset: { x: 585, y: -513 } }
  }), []);
  ...
  const handleImageMouseMove = (event) => {
    ...
    const inClickableArea = isPointInTvMove5Area(4, mouseX, mouseY, rect.width, rect.height);
    
    if (inClickableArea) {
      // setHoveredImage(4); // This line is removed
    } else {
  ...
  const renderClickableAreasDebug = () => {
    ...
    const areas = getTvMove5ClickableAreas(4);
  ...
  return (
    <div className="relative w-full h-screen" style={{ backgroundColor: '#1D1C1A' ,overflow: 'visible' }}>
      ...
      >
        <img 
          src={image || "/tv/pictures/tv2/move-5/regular/04.png"}
          alt="Full Screen Scene"
          className="w-full h-full object-cover"
        />
        {renderClickableAreasDebug()}
        {/* Add clickable area overlay */}
        {getTvMove5ClickableAreas(4).map((area, index) => (
          <div
            key={`clickable-${index}`}
        ...
        >
          <div className="relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col items-center">
              {/* Image */}
              <img
                src={zoomImage || "/tv/pictures/tv2/move-5/zoomIn/04.png"}
                alt="Zoomed Scene"
                style={{
                  width: '919px',
                  height: '181px',
